...
struct PhysicalProfileStep: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: Spacing.xl) {
                ...
                VStack(alignment: .leading, spacing: Spacing.m) {
                    ...
                    VStack(spacing: Spacing.s) {
                        ForEach(AverageMileage.allCases, id: \.self) {
                            mileage in
                            MileageSelectionRow(
                                mileage: mileage,
        ...
        HStack(spacing: Spacing.m) {
            ...
            VStack(spacing: Spacing.xs) {
                ...
                .onChange(of: heightFeet) { _, newValue in
                    profile.height = Height.fromImperial(
                        feet: newValue,
                        inches: heightInches
                    )
                }
    ...
    private var heightMetricPicker: some View {
        VStack(spacing: Spacing.xs) {
            ...
            Picker("Height", selection: $heightCentimeters) {
                ForEach(
                    Array(stride(from: 140, through: 220, by: 1)),
                    id: \.self
                ) { cm in
    ...
    private var weightImperialPicker: some View {
        VStack(spacing: Spacing.xs) {
            ...
            Picker("Weight", selection: $weightPounds) {
                ForEach(
                    Array(stride(from: 80, through: 350, by: 1)),
                    id: \.self
                ) { lbs in
    ...
    private var weightMetricPicker: some View {
        VStack(spacing: Spacing.xs) {
            ...
            Picker("Weight", selection: $weightKilograms) {
                ForEach(
                    Array(stride(from: 35, through: 160, by: 1)),
                    id: \.self
                ) { kg in
    ...
    private func updateHeightWeight() {
        if profile.measurementSystem == .imperial {
            ...
            let height = Height.fromImperial(
                feet: heightFeet,
                inches: heightInches
            )
...
struct MeasurementSystemButton: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .background(
                isSelected
                    ? Color.primaryOrange.opacity(0.1) : Color.cardBackground
            )
